...
class Source(ABC):
    ...
    def __init__(self, paths, pbar, timer, analysis, project_indicator):
        """
        .. warning::
            **Do not construct Source manually**
            Please use :meth:`SourceFactory.create`
        """
        self.paths = paths
        self.size = len(paths)
    ...
    @staticmethod
    def task_from_source(source):
        # type: (Source) -> Tuple[Dict[str,List[str]],str,int]
        source.read()
        source.seed()
        return source.msgs, source.project_link, source.migration_count
                ...
                if task:
                    ...
                    if ARCHIVED_TAG not in task_tags:
                        ...
                        self.msgs["FAILED"].append(
                            ...
                            f"you can archive it in Allegro Trains"
                        )
                        self.pbar.update(1)
                        continue
    ...
    def seed(self):
        for id in self.get_ids():
            ...
            self.call_func(
                "transmit_metrics", id, lambda id_: self.transmit_metrics(id_), id
            )

            self.call_func(
                "transmit_artifacts", id, lambda id_: self.transmit_artifacts(id_), id
            )
        ...
        for graph_name, series_name, table in metrics:
            for p in table:
                logger.report_scalar(
                    graph_name, series_name, iteration=p[0], value=float(p[1])
                )
        task.completed()
